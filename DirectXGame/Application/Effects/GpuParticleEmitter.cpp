...
void GpuParticleEmitter::Initialize(uint32_t maxParticles) {
    // 最低１は確保する
...
void GpuParticleEmitter::SetColor(float r, float g, float b, float a) {
    color_ = { r, g, b, a };
}
void GpuParticleEmitter::Emit(const Vector3& position, const Vector3& velocity, float life, float startScale, float endScale) {
    if (particles_.empty()) {
        return;
    }

    // 次の書き込み位置取得
    Particle& p = particles_[nextSpawnIndex_];
...
void GpuParticleEmitter::Update(float dt) {
    if (dt <= 0.0f) {
        return;
    }
    ...
    if (mappedParticles_) {
        std::memcpy(mappedParticles_, particles_.data(),
            sizeof(Particle) * particles_.size());
    }
}
...
void GpuParticleEmitter::Draw(const Camera* camera) {
    ...
    if (mappedConstants_) {
        ...
        std::copy(color_.begin(), color_.end(), std::begin(mappedConstants_->color));
    }
...
void GpuParticleEmitter::CreatePipeline_() {
    ...
    HRESULT hr = D3DCompileFromFile(
        L"./Resources/shaders/GpuParticleVS.hlsl",
        nullptr,
    ...
    hr = D3DCompileFromFile(
        L"./Resources/shaders/GpuParticlePS.hlsl",
        nullptr,
        D3D_COMPILE_STANDARD_FILE_INCLUDE,
        "main",
        "ps_5_0",
...
void GpuParticleEmitter::CreateVertexBuffer_() {
    ID3D12Device* device =
...
void GpuParticleEmitter::CreateParticleBuffer_() {
    ID3D12Device* device =
        DirectXCommon::GetInstance()->GetDevice();
    if (!device) {
        return;
    ...
    const UINT bufferSize =
        static_cast<UINT>(sizeof(Particle) * particles_.size());
...
void GpuParticleEmitter::CreateConstantBuffer_() {
    ID3D12Device* device =
        DirectXCommon::GetInstance()->GetDevice();
    if (!device) {
        return;
