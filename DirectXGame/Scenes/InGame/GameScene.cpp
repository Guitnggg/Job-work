...
void GameScene::Initialize() {
    ...
    // ===== ダメージ演出 =====
    damageEmitter_ = std::make_unique<GpuDamageEmitter>();
    damageEmitter_->Initialize(512);
...
void GameScene::SpawnDamageParticles() {
    ...
    for (int i = 0; i < kDamageParticleCount_; i++) {
        ...
        damageEmitter_->Emit(pos, vel, kDamageParticleLife_, kDamageParticleStartScale_, kDamageParticleEndScale_);
    }
}
...
void GameScene::StartExplosionAtPlayer(float scale) {
    ...
    if (damageEmitter_) {
        for (int i = 0; i < count; i++) {
            ...
            damageEmitter_->Emit(pos, vel, kDamageParticleLife_ * 1.5f, kDamageParticleStartScale_ * scale, 0.0f);
        }
    }
    if (railCamera_) {
