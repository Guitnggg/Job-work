...
void GameScene::UpdateLoacOnMakers() {
    ...
    // 
    if (result_ != GameResult::None || state_ != GameState::Playing) { return; }

    const Camera* cam = railCamera_ ? railCamera_->GetCamera() : &camera_;  // 

    // 
    if (!cam || lockOnTexHandle_ == 0u) { return; }
    ...
    for (CharacterBase* target : targets) {
        ...
        const Vector2 screenPos = {
            (clip.x * 0.5f + 0.5f) * kScreenWidth_,
            (-clip.y * 0.5f + 0.5f) * kScreenHeight_
        };
